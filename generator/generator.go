...
func (g *Generator) AddNodeSet(absoluteIndex, relativeIndex int, nc config.NodeConfig, ns types.NodeSet, fc *types.Faucet) (*types.NodeSet, error) {
	...
	initNodeSet, err := g.initiateNodeSet(absoluteIndex, relativeIndex, ns.GroupIndex, *cnc)
	if err != nil {
		return nil, err
	}
