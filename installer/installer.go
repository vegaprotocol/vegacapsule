...
func (i Installer) Install(ctx context.Context, releaseTag string, requestedAssets map[string]string) (map[string]string, error) {
	log.Printf("Starting to install binaries to %q", i.binDirectory)

	downloadAssets, err := i.getAssets(ctx, releaseTag, requestedAssets)
	if err != nil {
		return nil, err
	}

	var mut sync.Mutex
	installedBinsPaths := map[string]string{}
